 ...
 * - Hash navigation (#slide-5 jumps to 5th slide)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Guard: If the script is placed in <head> (instead of after </deck-stage>),
      // the parser might not have finished processing child <section> elements yet, causing querySelectorAll to return empty.
      // Defer to the next event loop cycle to ensure all child nodes are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document is still parsing, wait for DOMContentLoaded to process all sections at once
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document has finished parsing (script at the bottom of body or deferred), collect in the next frame
