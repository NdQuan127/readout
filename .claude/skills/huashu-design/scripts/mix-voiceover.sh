#!/usr/bin/env bash
# mix-voiceover.sh · Mix voiceover (vocal track) + optional BGM into an MP4
#
# Usage:
#   bash mix-voiceover.sh <video.mp4> --voiceover=<voice.mp3> [options]
#
# Required:
#   --voiceover=<path>    Path to voiceover mp3 (vocal track, generated by narrate-pipeline.mjs)
#
# Optional:
...
#   --no-ducking          Disable sidechain ducking (enabled by default: BGM ducks when voice is active)
#   --voice-volume=<0-2>  Voice volume multiplier, default 1.0
#   --out=<path>          Output path, default <input>-voiced.mp4
#
# Behavior:
#   - Video stream copied (no re-encoding, fast)
#   - Voiceover is always the main track, required; BGM is optional
#   - Sidechain ducking enabled by default: BGM compressed to approx -10dB when voice is active, recovers when voice stops
#   - Output length matches video length (padded with silence if audio is short; truncated if too long)
#
# Examples:
