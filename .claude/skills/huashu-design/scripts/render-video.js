...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: actively reset the animation time to zero - to handle
    // cases where the HTML does not strictly follow the starter tick template (e.g.,
    // lastTick uses performance.now() which includes font load time in the first frame's dt).
