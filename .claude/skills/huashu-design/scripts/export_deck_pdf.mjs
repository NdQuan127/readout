 ...
 *   - Backgrounds/graphics are 1:1 high-fidelity (Playwright embedded Chromium rendering)
 *   - No modifications needed for the HTML files
 *   - Zero visual loss (PDF is printed directly from the browser)
 *
 * Trade-offs:
 *   - PDF text is not editable (must edit HTML to modify)
 *
 * Dependencies: playwright pdf-lib
 *   npm install playwright pdf-lib
 *
 * Files will be sorted by filename (01-xxx.html → 02-xxx.html → ...)
 */
