defmodule ReadoutWeb.UserAuthTest do
  use ReadoutWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Readout.Accounts
  alias Readout.Accounts.Scope
  alias ReadoutWeb.UserAuth

  import Readout.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ReadoutWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ReadoutWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ReadoutWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
